...
public class HelloWorldTests {
	...
	//部署完成说明工作流引擎中已经有这个流程了
	//同样的一个流程，再次提交部署的时候，ACT_RE_PROCDEF表中的版本号是会递增的，通过版本号进行管理。默认走版本号高的进行
	@Test
	public void deployment() throws Exception {
		...
		Deployment deploy = this.repositoryService.createDeployment()
		  ...
		  .name(HELLO_WORLD_PROCESSDEFINITIONKEY)  //部署名字
		  .deploy();
...
//		Deployment deployment = this.repositoryService.createDeploymentQuery().deploymentId("1").singleResult();
...
//		}
		...
		//删除
		//this.repositoryService.deleteDeployment("5001");
...
//		System.err.println("processInstanceId: " + processInstance.getId());
	...
	public void completeTaskLevel_1() throws Exception {
		...
		for(Task task : taskList) {
			...
			Map<String, Object> variables = new HashMap<String, Object>();
			variables.put("info", "又又同意了...");
			
			//绑定跟当前这个任务相关的参数信息: (绑定到历史任务相关的表中)
	...
	//在工作流里面没有执行成功和执行失败(这是业务方面的事情)  只有任务有没有被完成
	@Test
